...
export default function UserDashboard({ currentUser, theatres, events, seatsByEvent, bookings, onBookSeat, onPayNow, notifications }) {
  ...
  React.useEffect(() => {
    // Only auto-select area when NOT actively searching (to avoid clearing on zero-match search)
  ...
  }, [selectedArea, areaOptions, searchText]);
  ...
  React.useEffect(() => {
    // Only auto-select event when NOT actively searching
    if (searchText.trim()) return;
  ...
  }, [selectedEvent, theatreEvents, searchText]);

  const selectedEventInfo = theatreEvents.find((e) => toNumber(e.eventId) === toNumber(selectedEvent));
  const pendingBooking = bookings.find(
    (b) => b.userId === currentUser.userId && b.status === 'PENDING_PAYMENT'
  );
  const paidBooking = bookings.find(
    (b) => b.userId === currentUser.userId && b.status === 'PAID'
  );
  const userBooking = pendingBooking || paidBooking;
                ...
                <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5}>
                  ...
                  {userBooking && (
                    <Button
                      ...
                      disabled={userBooking.status !== 'PENDING_PAYMENT' || isPaying}
                    >
            ...
            <CardContent sx={{ pt: 3, height: '100%', display: 'flex', flexDirection: 'column', color: '#eef4ff' }}>
              ...
              <Box sx={{ display: 'flex', justifyContent: 'center', mt: 1.8 }}>
                <Button
                  ...
                  disabled={!selectedSeat || !selectedEvent || isBooking}
                  sx={{
                    minWidth: 260,
                    borderRadius: 2,
                    py: 1.25,
